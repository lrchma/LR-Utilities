...
var options = {
        nodes: {
			...
			font: { size: '20' ,color:'#002C52', face:'arial', background: '#ffffff'},
          shape: 'dot',
          scaling: {
		...
		layout: {
          hierarchical: {
            sortMethod: "directed",
			levelSeparation: 400			//Change vertical distance between nodes
          }
        },
        physics: {
            enabled: true,
    ...
    switch ($component.type)
        {
            PM {  
                if ($component.heartbeat -gt $utctime.AddMinutes(-5)) { #Heartbeat OK
                    $a = "`{{id: {0}, label: '{1}:{2} \n Event Rate: {3}', color: '#8DA05C', level: '0', group: 0, title: '{4}', value: {5}}}," -f $component.id, $component.type, $component.name, $component.eps, $component.heartbeat, $component.eps
                    Add-Content $outputfile $a
                }elseif($component.heartbeat -gt $utctime.AddMinutes(-10)){ #Hearbeat late
                    $a = "`{{id: {0}, label: '{1}:{2} \n Event Rate: {3}', color: '#DBAE65', level: '0', group: 0, title: '{4}', value: {5}}}," -f $component.id, $component.type, $component.name, $component.eps, $component.heartbeat, $component.eps
                    Add-Content $outputfile $atctim
                }
                else{ #heartbeat error
                    $a = "`{{id: {0}, label: '{1}:{2} \n Event Rate: {3}', color: '#B45034', level: '0', group: 0, title: '{4}', value: {5}}}," -f $component.id, $component.type, $component.name, $component.eps, $component.heartbeat, $component.eps
                    Add-Content $outputfile $a
                }

                }           
            DP {  
                $a = "{{id: {0}, label: '{1}:{2} \n Index Rate: {3} \n Receive Rate: {4}', color: '#8DA05C', level: '1', group: 1, title:'{5}', value: {6}}}," -f $component.id,$component.type,$component.name,$component.mpsOnline,$component.mpsCount, $component.heartbeat, $component.mpsCount
                Add-Content $outputfile $a 
                }

            SM {
                if ($component.heartbeat -gt $utctime.AddMinutes(-5)) { #Heartbeat OK
                $a = "{{id: {0}, label: '{1}:{2} \n Recieve Rate: {3}', color: '#8DA05C', level: '2', group: 2, title:'{4}', value: {5}}}," -f $component.id,$component.type,$component.name,$component.lps, $component.heartbeat,$component.lps
                Add-Content $outputfile $a
                }elseif($component.heartbeat -gt $utctime.AddMinutes(-10)){ #Hearbeat late
                $a = "{{id: {0}, label: '{1}:{2} \n Recieve Rate: {3}', color: '#DBAE65', level: '2', group: 2, title:'{4}', value: {5}}}," -f $component.id,$component.type,$component.name,$component.lps, $component.heartbeat,$component.lps
                Add-Content $outputfile $a
                }
                else{ #heartbeat error
                $a = "{{id: {0}, label: '{1}:{2} \n Recieve Rate: {3}', color: '#B45034', level: '2', group: 2, title:'{4}, value: {5}}}," -f $component.id,$component.type,$component.name,$component.lps, $component.heartbeat,$component.lps
                Add-Content $outputfile $a
                }
                        ...
                        $component = $components[$id]                   
                            switch ($component.type) {
                                DP {
                                        ...
                                        write-output "{from: $PM, to: $DP, value: $dpMPS, color: '#007DC3' }," | Add-Content $outputfile
                                     }
                            }
                    }
                }
                        ...
                        $component = $components[$id]                   
                            switch ($component.type) {
                                SM {
                                        $SM = $component.id
                                        $smLPS = $component.lps
                                        write-output "{from: $DP, to: $SM, value: $smLPS, color: '#007DC3'}," | Add-Content $outputfile
                                     }
                            }
